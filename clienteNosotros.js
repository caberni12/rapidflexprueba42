(async () => {
  try {
    ...
    // Cargar Excel
    const resp = await fetch('datos_nosotros.xlsx');
    ...
    // Al cargar la vista 'nosotros' en la SPA
    document.addEventListener('spa:view-loaded', (e) => {
      if (e?.detail?.key === 'nosotros') applyValues(e.detail.root || document);
    });
  } catch (err) {
    console.error('Error al procesar el Excel (nosotros):', err);
  }
})();
