...
(async () => {
  try {
    ...
    const resp = await fetch('datos_menu.xlsx', { cache: 'no-store' });
    ...
    document.addEventListener('spa:view-loaded', (e) => {
      if (e?.detail?.key === 'menu') applyValues(e.detail.root || document);
    });
  } catch (err) {
    console.error('Error al procesar el Excel del menú:', err);
  }
})();
